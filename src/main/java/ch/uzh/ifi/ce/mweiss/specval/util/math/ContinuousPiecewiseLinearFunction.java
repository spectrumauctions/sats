 ...
 */
package ch.uzh.ifi.ce.mweiss.specval.util.math;
...
public final class ContinuousPiecewiseLinearFunction implements Function{
    ...
    public List<SimpleImmutableEntry<BigDecimal, BigDecimal>> getCornerPoints(){
        ...
        result.add(new SimpleImmutableEntry<BigDecimal, BigDecimal>(lowestX, firstY));
        for(BigDecimal functionUpperX : linearFunctions.keySet()){
            BigDecimal y = getY(functionUpperX);
            result.add(new SimpleImmutableEntry<BigDecimal, BigDecimal>(functionUpperX, y));
        }
        return result;
    }

    /* (non-Javadoc)
     * @see ch.uzh.ifi.ce.mweiss.specval.util.math.Function#getY(java.math.BigDecimal)
     */
    @Override
