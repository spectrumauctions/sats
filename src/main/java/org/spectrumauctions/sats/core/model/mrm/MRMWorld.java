...
public class MRMWorld extends World {
     ...
     * @return
     */
    public static BigDecimal c(MRMRegionsMap.Region r, Bundle<MRMLicense> bundle){
        if(bundle.isEmpty()){
    ...
    public BigDecimal getMaximumRegionalCapacity() {
        if(maximalRegionalCapacity == null){
            MRMRegionsMap.Region anyRegion = regionsMap.getRegions().iterator().next();
     ...
     * @return
     */
    public static Map<MRMRegionsMap.Region, Bundle<MRMLicense>> getLicensesPerRegion(Bundle<MRMLicense> bundle){
        Preconditions.checkArgument(!bundle.isEmpty());
        MRMWorld world = bundle.iterator().next().getWorld();
