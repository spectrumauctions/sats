...
public enum SupportVectorSetupFactory {
	...
	public SupportVectorSetup createDefaultGSVMSupportVectorSetup(KernelType kernelType) {
		return new SupportVectorSetup(3000d, 0.05d, BigDecimal.ONE, this.gsvmDefaultKernels.get(kernelType));
	}
	
	public SupportVectorSetup createDefaultLSVMSupportVectorSetup(KernelType kernelType) {
		return new SupportVectorSetup(100d,0.0001d,BigDecimal.ONE,this.lsvmDefaultKernels.get(kernelType));
	}
	
	public SupportVectorSetup createDefaultMRVMSupportVectorSetup(KernelType kernelType) {
		return new SupportVectorSetup(100d,0.0001d,BigDecimal.ONE.divide(BigDecimal.valueOf(100000)),this.mrvmDefaultKernels.get(kernelType));
	}
