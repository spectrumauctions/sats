...
public class MRMWorldTest {
    ...
    public static void setUpBeforeClass(){
        world = new MRMWorld(MRMSimpleWorldGen.getSimpleWorldBuilder(),new JavaUtilRNGSupplier(983742L));
    ...
    public void selectAllLicensesOfRegionOnCompleteBundle(){
        Map<MRMRegionsMap.Region, Bundle<MRMLicense>> regionalBundles = MRMWorld.getLicensesPerRegion(completeBundle);
        ...
        for(MRMRegionsMap.Region region : world.getRegionsMap().getRegions()){
            ...
            for(MRMLicense license : regionalBundles.get(region)){
    ...
    public void selectAllLicensesOfBandOnCompleteBundle(){
        Map<MRMBand, Bundle<MRMLicense>> regionalBundles = MRMWorld.getLicensesPerBand(completeBundle);
        for(MRMBand band : world.getBands()){
            ...
            for(MRMLicense license : regionalBundles.get(band)){
    ...
    public void capacityIsCorrectlyCalculatedCompleteRegionalBundle(){
        Map<MRMRegionsMap.Region, Bundle<MRMLicense>> regionalBundles = MRMWorld.getLicensesPerRegion(new Bundle<>(world.getLicenses()));
        ...
        for(Entry<MRMRegionsMap.Region, Bundle<MRMLicense>> regionalBundle : regionalBundles.entrySet()){
            BigDecimal bundleRegionalCapacity = MRMWorld.c(regionalBundle.getKey(), regionalBundle.getValue());
    ...
    public void regionalDistanceTests(){
        MRMRegionsMap.Region anyRegion = world.getRegionsMap().getRegions().iterator().next();
        for(MRMRegionsMap.Region r : world.getRegionsMap().getRegions()){
