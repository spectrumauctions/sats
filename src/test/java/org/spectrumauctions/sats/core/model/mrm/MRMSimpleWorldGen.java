...
public class MRMSimpleWorldGen {
    ...
    static MRMWorldSetup getSimpleWorldBuilder(){
        MRMWorldSetup.MRMWorldSetupBuilder builder = new MRMWorldSetup.MRMWorldSetupBuilder();
        builder.createGraphRandomly(new IntegerInterval(5), new IntegerInterval(2), 100, 0);
        List<MRMWorldSetup.BandSetup> setups = new ArrayList<>(builder.bandSetups().values());
        for(MRMWorldSetup.BandSetup bandSetup : setups ){
            builder.removeBandSetup(bandSetup.getName());
        }       
        //Band with 2 lots and base capacity 20, no synergies
        builder.putBandSetup(new MRMWorldSetup.BandSetup(
                "TEST_A", 
